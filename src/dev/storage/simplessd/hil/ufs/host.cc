...
void Host::processUTPTask() {
  // Not Implemented
  ssd_panic("UTP Task is not implemented");
}
  ...
  DMAFunction doRequest = [this](uint64_t now, void *context) {
    ...
    switch (reqType) {
      ...
      case OPCODE_COMMAND:
        ...
        break;
      default:
        ssd_panic("Unapplicable UPIU command type");
...
void Host::writeRegister(uint32_t offset, uint32_t data, uint64_t tick) {
  ...
  switch (offset) {
    ...
    case REG_UCMDARG3:
      ...
      break;
    default:
      ssd_warn("Write to read only register 0x%X", offset);
    ...
    DMAFunction doRead = [this](uint64_t, void *context) {
      ...
      switch (ct) {
        ...
        case CMD_DEVICE_MGMT_FUNCTION:
          ...
          break;
        default:
          ssd_panic("Undefined UTP Transfer command type %#x", ct);
