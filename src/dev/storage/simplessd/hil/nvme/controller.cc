  ...
  if (vid == OCSSD_VENDOR) {
    ...
    switch (ssvid) {
      ...
      case OCSSD_SSVID_2_0:
        ...
        break;
      default:
        ssd_panic("nvme_ctrl: Invalid SSVID for Open-Channel SSD");
  ...
  if (size == 4) {
    ...
    switch (offset) {
      ...
      case REG_ADMIN_SQUEUE_BASE_ADDR + 4:
        ...
        break;
      default:
        ssd_panic("nvme_ctrl: Write on read only register");
        break;
    }
    ...
    ssd_panic("nvme_ctrl: Invalid read size(%d) on controller register", size);
  }
...
void Controller::collectSQueue(DMAFunction &func, void *context) {
  ...
  // Weighted round robin
    ...
    ssd_panic("nvme_ctrl: Invalid arbitration method");
  }
...
void Controller::submit(CQEntryWrapper &entry) {
  ...
  if (pQueue == NULL) {
    ssd_panic("nvme_ctrl: Completion Queue not created! CQID %d", entry.cqID);
  }
